...
!     print *, 'jat = ',jat, ' iat= ', iat
     ...
     !   call Logfile_stampMessage(myPE,'[eos_helm] Negative total pressure.')
     !   if (myPE == MASTER_PE) print *,'[eos_helm] Negative total pressure.'

     !   if ( s4 > 0.e0 ) then
     !      write(internalFile,'(a,3es12.3)')  &
     !&           ' caused by coulomb correction. s,x: ',s4,x6,abar
     !      call Logfile_stampMessage(myPE,internalFile)
     !      if (myPE == MASTER_PE) print *, internalFile
     !      ! Better diagnostics from some simulation which shall remain  nameless    
     ...
     !         call Logfile_stampMessage(myPE,internalFile)
     !         if (myPE == MASTER_PE) print *, internalFile
     !         call Logfile_stampMessage(myPE,'  It is likely that the mesh is of low quality.')
     !         if (myPE == MASTER_PE) print *, '      It is likely that the mesh is of low quality.'
     !         if ( eos_coulombAbort) call Driver_abortFlash &
     !              ('[eos_helm] ERROR: abar is negative.')
     !      else
     !         if ( eos_coulombMult > 0.e0 ) then
     !            call Logfile_stampMessage (myPE,'  set runtime parameter eos_coulombMult to zero')
     !            if (myPE == MASTER_PE) print *, '  set runtime parameter eos_coulombMult to zero'
     !         end if
     !         if ( eos_coulombAbort) then
     !            call Driver_abortFlash('[eos_helm] ERROR: coulomb correction causing negative total pressure.')
     !         else
     !            call Logfile_stampMessage(myPE,'Setting coulombMult to zero for this call, eos_coulombAbort=false.')
     !            if (myPE == MASTER_PE) print *, '  Setting coulombMult to zero for this call, eos_coulombAbort=false'
     ...
     if (pres .le. 0.0 .or. ener .le. 0.0 .or. srad .le. 0.0) then
         pcoul    = 0.0d0
         dpcouldd = 0.0d0
         dpcouldt = 0.0d0
         ecoul    = 0.0d0
...
#ifdef DEBUG_EOS
     if (ptotRow(j) .ne. ptotRow(j)) then
        print *, btemp, den, abar, zbar
        print *, prad, pion, pele, pcoul
        call Driver_abortFlash('NaN pres in eos_helm')
     endif
#endif
